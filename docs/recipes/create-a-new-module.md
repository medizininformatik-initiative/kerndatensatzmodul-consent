...
3. **Fill the placeholders.** Open `sushi-config.yaml` and replace every `{{…}}`
   ...
   - `consent` — lowercase short name (`person`), drives packageId/id/canonical.
   - `Consent` — CamelCase (`Person`) → `name: MII_IG_Person`.
   - `Consent` — the human-readable title part (`Person`), which becomes
     `title: MII Implementation Guide Person`.
   - `2026.0.0` — `YYYY.n.n` (e.g. `2027.0.0`; drafts/pre-releases add a
...
4. **Rename the three placeholder-NAMED files.** Three file names carry a
   ...
   git mv 'input/pagecontent/ImplementationGuide-mii-ig-consent.md' \
          'input/pagecontent/ImplementationGuide-mii-ig-person.md'
   git mv 'input/translations/de/pagecontent/ImplementationGuide-mii-ig-consent.md' \
