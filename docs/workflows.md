...
- **The reusable validation needs two files in the repo root**, at fixed paths the
  ...
  carries `consent`/`Consent`/`2026.0.0` placeholders
...
| **The render names its release** | The demo's `2026.0.0` is substituted by `scripts/self-check-substitute.sh` with `SELF_CHECK_RELEASE_TAG` set, producing `2027.0.0-template.<tag without v>`. After the build the generated `ImplementationGuide` is read back and the job **fails** unless it carries exactly that value. |
