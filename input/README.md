...
| `pagecontent/` | The IG's narrative pages as Markdown (`index.md`, `guidance.md`, …). Every page listed under `pages:` in `sushi-config.yaml` needs a file here. **German is the default content language** (DE-first, decision D-3); the English mirror lives in `translations/en/pagecontent/`. | IG Publisher |
