...
1. **Replace `consent` inside the file** — it appears in the resource
   `id` (`mii-param-consent-manifest`). A `{{…}}` left in a FHIR `id` is
   ...
   `input/translations/en/ImplementationGuide-mii-ig-consent.po`, which
