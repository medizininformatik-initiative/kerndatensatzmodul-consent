#!/usr/bin/env node
// DE-FIRST PATCH (migration decision D-3, same commit as the i18n flip): this
// module inverts the template language model — German default in
// input/pagecontent/, ENGLISH translation under input/translations/en/. Every
// translation-tree path below was swapped from translations/de accordingly.
    ...
    add(
      ...
      release
        ? "the demonstration page is still present on a release branch — remove all of: " +
          ...
          "input/translations/en/pagecontent/rendering-artifacts.md, " +
      ...
      add("M9 optional pages", "module", release ? "fail" : "pass",
        ...
        release
          ? "optional pages still carry their OPTIONAL-PAGE marker on a release branch — decide each one: " +
            ...
            "input/translations/en/pagecontent/<page>) or REMOVE it (follow the per-entry procedure in " +
      ...
      add("M11 illustrative examples", "module", release ? "fail" : "pass",
        ...
        release
          ? "pages still carry a scaffold ILLUSTRATIVE-EXAMPLE block on a release branch — delete the " +
            ...
            "input/translations/en/pagecontent/<page> (write the module's own content or adopt the " +
...
export function scanOptionalPages(root) {
  const dirs = {
    ...
    de: join(root, "input", "translations", "en", "pagecontent"),
  };
...
function scanDuplicateHeadings(root) {
  ...
  const titlesDe = {};
  try {
    const po = readdirSync(join(root, "input", "translations", "en"))
      .find((f) => /^ImplementationGuide-.*\.po$/.test(f));
    if (po) {
      const raw = readFileSync(join(root, "input", "translations", "en", po), "utf8");
  ...
  const dirs = [
    ...
    ["input/translations/en/pagecontent", titlesDe],
    ["input/intro-notes", null],
    ["input/translations/en/intro-notes", null],
  ];
  const problems = [];
